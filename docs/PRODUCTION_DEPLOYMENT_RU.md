...
sudo install -d -m 0750 /opt/genesisit/releases/1.0.23
sudo tar -xzf remoteit-1.0.23-source.tar.gz -C /opt/genesisit/releases/1.0.23
sudo ln -sfn /opt/genesisit/releases/1.0.23 /opt/genesisit/current
...
```sh
sha256sum -c remoteit-1.0.23-source.tar.gz.sha256
```
