...
RUN /go/bin/goversioninfo -64 -icon=assets/genesisit.ico -application-icon=assets/genesisit.ico -manifest=assets/genesisit.exe.manifest \
	...
	-file-version=1.0.42.0 -product-version=1.0.42.0 -product-name=RemoteIt \
      -internal-name=RemoteItAgent -original-name=RemoteIt-Agent-Setup.exe \
	  -ver-major=1 -ver-minor=0 -ver-patch=42 -ver-build=0 \
	-product-ver-major=1 -product-ver-minor=0 -product-ver-patch=42 -product-ver-build=0 assets/versioninfo.json && \
...
RUN --mount=type=secret,id=android_keystore \
    ...
    ${ANDROID_SDK_ROOT}/build-tools/36.0.0/apksigner verify --verbose --print-certs app/build/outputs/apk/release/app-release.apk > /tmp/app-signer.txt && \
    ${ANDROID_SDK_ROOT}/build-tools/36.0.0/apksigner verify --verbose --print-certs agentapp/build/outputs/apk/release/agentapp-release.apk > /tmp/agentapp-signer.txt && \
    APP_CERT="$(sed -n 's/^Signer #1 certificate SHA-256 digest: //p' /tmp/app-signer.txt | head -n 1)" && \
    AGENT_CERT="$(sed -n 's/^Signer #1 certificate SHA-256 digest: //p' /tmp/agentapp-signer.txt | head -n 1)" && \
    test -n "$APP_CERT" && test "$APP_CERT" = "$AGENT_CERT" && \
    printf 'RemoteIt Android release certificate\nSHA-256: %s\nApplies to: RemoteIt.apk, RemoteIt-Agent-Android.apk\n' "$APP_CERT" > /tmp/APK-SIGNER.txt
...
COPY --from=android-build /tmp/APK-SIGNER.txt /app/web/downloads/APK-SIGNER.txt
