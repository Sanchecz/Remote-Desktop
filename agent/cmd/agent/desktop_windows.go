...
var (
	...
	procOpenWindowStation       = user32Desktop.NewProc("OpenWindowStationW")
	procSetProcessWindowStation = user32Desktop.NewProc("SetProcessWindowStation")
	procCloseWindowStation      = user32Desktop.NewProc("CloseWindowStation")
...
var desktopInteractiveWindowStation uintptr

func bindDesktopProcessWindowStation() error {
	if desktopInteractiveWindowStation != 0 {
		return nil
	}
	...
	handle, _, openErr := procOpenWindowStation.Call(uintptr(unsafe.Pointer(name)), 0, maximumAllowed)
	if handle == 0 {
		return fmt.Errorf("open interactive Windows station: %w", openErr)
	}
	if applied, _, setErr := procSetProcessWindowStation.Call(handle); applied == 0 {
		procCloseWindowStation.Call(handle)
		return fmt.Errorf("bind capture to interactive Windows station: %w", setErr)
	}
	// The assigned process Window Station must stay open for the helper lifetime.
	// The OS closes the handle when this short-lived worker exits.
	desktopInteractiveWindowStation = handle
	return nil
}
