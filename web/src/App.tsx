...
import { imageClipboardFingerprint, newerRemoteClipboardPayload, REMOTE_CLIPBOARD_COPY_READ_DELAYS, REMOTE_CLIPBOARD_COPY_TIMEOUT, sameRemoteClipboardPayload, shouldAcceptRemoteClipboardSequence, shouldResolveRemoteClipboardCopy, textClipboardFingerprint, type RemoteClipboardCopyGate, type RemoteClipboardPayload, type RemoteClipboardSyncState } from "./remoteClipboard";
...
const LATEST_AGENT_VERSION = "1.0.43";
...
function DeviceLANAndPrinterTools({ device, csrf }: { device: Device; csrf: string }) {
	...
	return <section className="device-admin-tools">
		...
		{tab === "network" ? <div className="device-tool-body">
			<div className="lan-scan-line"><label><span>Подсети или диапазоны (необязательно)</span><input value={subnet} onChange={(event) => setSubnet(event.target.value)} placeholder="192.168.1.0/24; 192.168.2.1-192.168.2.254" /></label><button type="button" className="secondary-button" disabled={!!busy || !device.online} onClick={() => void scanNetwork()}>{busy === "diagnostic.lan_scan" ? <RefreshCw className="spin" size={16} /> : <Search size={16} />} Найти устройства</button></div>
...
function NetworkConnectionsPage({ devices, currentUser, csrf, onAccessChanged }: { devices: Device[]; currentUser: User; csrf: string; onAccessChanged: (deviceId: string, accessProtected: boolean, accessGranted: boolean) => void }) {
	...
	const [activeConnection, setActiveConnection] = useState<{ protocol: "rdp" | "ssh"; host: string; port: number; gatewayName: string } | null>(null);
	const [connectionFrameKey, setConnectionFrameKey] = useState(0);
	const [busy, setBusy] = useState(false);
	const [message, setMessage] = useState("");
	const [error, setError] = useState("");
	const connectionPanelRef = useRef<HTMLElement>(null);
	...
	async function connect(event: FormEvent) {
		...
		setBusy(true); setError(""); setMessage(""); setLaunchURL(""); setActiveConnection(null);
		try {
			...
			setActiveConnection({ protocol, host: internalHost.trim(), port, gatewayName: gateway.name });
			setConnectionFrameKey((current) => current + 1);
			setMessage(`${protocol.toUpperCase()} подготовлен внутри панели через Agent «${gateway.name}». На компьютере‑шлюзе окна и консоли не запускаются.`);
			setPassword("");
		} catch (reason) {
	...
	return <>
		...
		{launchURL && activeConnection && <section ref={connectionPanelRef} className="embedded-connection-panel">
			<header><div><span className="preview-live active"><span />LIVE</span><span><strong>{activeConnection.protocol.toUpperCase()} · {activeConnection.host}:{activeConnection.port}</strong><small>Встроенный защищённый сеанс через {activeConnection.gatewayName}</small></span></div><nav><button type="button" onClick={() => setConnectionFrameKey((current) => current + 1)} title="Переподключить встроенное окно"><RefreshCw size={16} /> Переподключить</button><button type="button" onClick={() => void connectionPanelRef.current?.requestFullscreen()} title="Развернуть сеанс"><Maximize2 size={16} /> Полный экран</button><a href={launchURL} target="_blank" rel="noreferrer" title="Резервно открыть в отдельной вкладке"><Link2 size={16} /> Новая вкладка</a><button type="button" className="danger" onClick={() => { setLaunchURL(""); setActiveConnection(null); setMessage("Сеанс закрыт в панели"); }}><X size={16} /> Закрыть</button></nav></header>
			<iframe key={connectionFrameKey} src={launchURL} title={`${activeConnection.protocol.toUpperCase()} ${activeConnection.host}`} allow="clipboard-read; clipboard-write; fullscreen" referrerPolicy="same-origin" />
		</section>}
	</>;
}
...
function PrintersPage({ devices, currentUser, csrf, onAccessChanged }: { devices: Device[]; currentUser: User; csrf: string; onAccessChanged: (deviceId: string, accessProtected: boolean, accessGranted: boolean) => void }) {
	...
	return <>
		<section className="page-heading">
			<div><span className="eyebrow">ПЕЧАТЬ И СКАНИРОВАНИЕ</span><h1>Принтеры</h1><p>Установленные принтеры и безопасное обнаружение сетевых устройств через выбранный Agent.</p></div>
			<button className="primary-button" disabled={!!busy || !gateway?.accessGranted} onClick={() => void discover()}>{busy ? <RefreshCw className="spin" size={17} /> : <Search size={17} />} Найти принтеры</button>
		</section>
		{gateway && !gateway.accessGranted && <DeviceAccessPanel device={gateway} currentUser={currentUser} csrf={csrf} onChanged={onAccessChanged} gate />}
		<section className="printer-control-panel">
			<div className="printer-discovery-toolbar">
				<label><span>Agent в нужной сети</span><select value={gatewayId} onChange={(event) => { setGatewayId(event.target.value); setResult(null); }}><option value="">Выберите Agent</option>{gateways.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.localIps?.[0] || item.publicIp}{item.accessGranted ? "" : " · требуется пароль"}</option>)}</select></label>
				<label><span>Подсеть (необязательно)</span><input value={subnet} onChange={(event) => setSubnet(event.target.value)} placeholder="автоматически или 192.168.1.0/24" /></label>
				<button type="button" className="secondary-button" disabled={!!busy || !gateway?.accessGranted} onClick={() => void simpleAction("windows.printers.open_settings", {}, "Настройки принтеров открыты на выбранном компьютере")}><Settings size={16} /> Настройки Windows</button>
				<button type="button" className="secondary-button" disabled={!!busy || !gateway?.accessGranted} onClick={() => void simpleAction("service.restart", { name: "Spooler" }, "Очередь печати перезапущена")}><RefreshCw size={16} /> Очередь печати</button>
			</div>
			{result ? <div className="printer-results-grid">
				<article><header><div><h2>Установлены на {gateway?.name}</h2><p>{installed.length} принтеров и очередей</p></div></header><div className="printer-device-list">
					{installed.map((printer) => <div key={printer.Name}><span className="printer-device-icon"><HardDrive size={18} /></span><span><strong>{printer.Name || "Без имени"}</strong><small>{printer.PortName || "порт не указан"} · {printer.DriverName || "драйвер не указан"}</small></span>{printer.Default ? <b>по умолчанию</b> : printer.Name && <button type="button" onClick={() => void simpleAction("windows.printer.set_default", { name: printer.Name }, `Принтер «${printer.Name}» назначен по умолчанию`)}>По умолчанию</button>}</div>)}
					{installed.length === 0 && <div className="printer-empty"><HardDrive size={24} /><span>Установленных принтеров нет</span></div>}
				</div></article>
				<article><header><div><h2>Найдены в сети</h2><p>{result.subnet} · Agent {result.agentIp} · веб-панель открывается на удалённом ПК</p></div></header><div className="printer-device-list">
					{(result.network || []).map((printer) => <div key={printer.ip}><span className="printer-device-icon network"><Wifi size={18} /></span><span><strong>{printer.name || printer.ip}</strong><small>{printer.name ? `${printer.ip} · ` : ""}{printer.services.join(" · ")}</small></span>{printer.web?.length > 0 ? <button type="button" title={`Открыть ${printer.web[0]} на удалённом компьютере`} onClick={() => { const url = new URL(printer.web[0]); void simpleAction("windows.printer.open_web", { host: printer.ip, scheme: url.protocol.replace(":", "") }, `Веб-панель ${printer.ip} открыта на удалённом компьютере`); }}><Link2 size={14} /> Веб-панель</button> : <small className="printer-web-unavailable">Веб-панель не найдена</small>}</div>)}
					{!(result.network || []).length && <div className="printer-empty"><Search size={24} /><span>Сетевые службы печати не найдены</span></div>}
				</div></article>
			</div> : <div className="printer-intro"><span className="stat-icon green"><HardDrive size={23} /></span><div><h2>Выберите Agent и запустите поиск</h2><p>RemoteIt проверит одну локальную подсеть, стандартные порты печати и уже установленные очереди. Другие порты и внешние адреса не сканируются.</p></div></div>}
			{message && <div className="form-success">{message}</div>}{error && <div className="form-error">{error}</div>}
		</section>
	</>;
}
...
function NetworkScanPage({ devices, csrf }: { devices: Device[]; csrf: string }) {
	...
	return <><section className="page-heading"><div><span className="eyebrow">ЛОКАЛЬНАЯ СЕТЬ</span><h1>Сканер сети</h1><p>Проверка доступных внутренних IP через Agent выбранного пользователя без установки VPN.</p></div><span className="connection-security-badge"><ShieldCheck size={17} /> До 1024 частных IP</span></section><article className="settings-card settings-card-wide settings-network-card network-page-card"><div className="settings-card-head"><span className="stat-icon green"><Search size={20} /></span><div><h2>Найти устройства и службы</h2><p>Можно объединить до восьми CIDR, отдельных IP или диапазонов IP-IP; Agent исключает повторы и проверяет только внутренние адреса</p></div></div><div className="settings-network-form"><label><span>Пользователь и Agent</span><select value={gatewayId} onChange={(event) => { setGatewayId(event.target.value); setResult(null); }}><option value="">Выберите устройство</option>{grouped.map(([userName, items]) => <optgroup key={userName} label={userName}>{items.map((device) => <option value={device.id} key={device.id}>{device.name} · {device.localIps?.[0] || device.publicIp}</option>)}</optgroup>)}</select></label><label><span>Подсети и диапазоны</span><input value={subnet} onChange={(event) => setSubnet(event.target.value)} placeholder="192.168.1.0/24; 192.168.2.1-192.168.2.254" /><small>Оставьте пустым для рекомендуемой сети Agent</small></label><button type="button" className="primary-button" disabled={loading || !gateway} onClick={() => void scan()}>{loading ? <RefreshCw className="spin" size={16} /> : <Search size={16} />} Просканировать</button></div>{result?.availableSubnets && result.availableSubnets.length > 1 && <div className="subnet-suggestions"><span>Сети Agent:</span>{result.availableSubnets.map((item) => <button type="button" key={item.subnet} className={(result.ranges || [result.subnet]).includes(item.subnet) ? "active" : ""} onClick={() => setSubnet((current) => current.trim() ? `${current.trim()}; ${item.subnet}` : item.subnet)}>{item.subnet}<small>{item.interface}{item.preferred ? " · рекомендуется" : ""}</small></button>)}</div>}{result && <div className="lan-scan-results"><div><strong>{(result.ranges || [result.subnet]).join(" · ")}</strong><span>Agent {result.agentIp} · проверено {result.scanned} · {result.durationMs} мс</span></div>{result.hosts.length ? result.hosts.map((item) => <article key={item.ip}><span className="lan-host-state" /><div><strong>{item.name || item.ip}</strong><small>{item.name ? `${item.ip} · ` : ""}{item.openPorts.length ? `порты: ${item.openPorts.join(", ")}` : "узел отвечает"}</small></div><span>{item.latencyMs} мс</span></article>) : <p>Отвечающие устройства не обнаружены.</p>}</div>}{error && <div className="form-error">{error}</div>}</article></>;
}
...
async function normalizeClipboardImage(blob: Blob): Promise<Blob> {
	if (blob.type === "image/png") return blob;
	if (!blob.type.startsWith("image/")) throw new Error("Буфер содержит неподдерживаемый формат изображения");
	...
	try {
		...
		canvas.width = bitmap?.width || fallbackImage?.naturalWidth || 0;
		canvas.height = bitmap?.height || fallbackImage?.naturalHeight || 0;
		if (canvas.width < 1 || canvas.height < 1) throw new Error("Изображение буфера не имеет допустимого размера");
		const context = canvas.getContext("2d", { alpha: true });
		if (!context) throw new Error("Браузер не смог преобразовать изображение буфера");
		context.drawImage(bitmap || fallbackImage!, 0, 0);
		return await new Promise<Blob>((resolve, reject) => canvas.toBlob(
			(result) => result ? resolve(result) : reject(new Error("Браузер не смог сформировать PNG")),
			"image/png",
		));
	} finally {
		bitmap?.close();
	...
	if (navigator.clipboard.read) {
		try {
			...
			for (const item of items) {
				...
				if (imageType) {
					image = await normalizeClipboardImage(await item.getType(imageType));
					break;
				}
			}
			if (!image) {
			...
			// Firefox and managed Chromium can expose read() but allow only the text
			// method. Retain the interoperable path before surfacing a permission error.
...
async function localClipboardSnapshotFingerprint(snapshot: LocalClipboardSnapshot): Promise<string> {
	if (snapshot.image) return imageClipboardFingerprint(await clipboardBlobFingerprint(snapshot.image));
	return textClipboardFingerprint(snapshot.text);
}
...
function RemoteDesktopModal({ device, csrf, initialSessionId = "", onClose, embedded = false }: { device: Device; csrf: string; initialSessionId?: string; onClose: () => void; embedded?: boolean }) {
  ...
  const [starting, setStarting] = useState(true);
	...
	const clipboardLastLocalFingerprintRef = useRef("");
	const clipboardLastSentToRemoteFingerprintRef = useRef("");
	const clipboardRemoteObservedFingerprintRef = useRef("");
	...
	const clipboardLastRemoteWindowsSequenceRef = useRef(0);
	const clipboardLastRemoteTextRef = useRef<string | null>(null);
	const clipboardPendingRemoteRef = useRef<RemoteClipboardPayload | null>(null);
	const clipboardPendingRemoteOrderRef = useRef(0);
	const clipboardRemoteWriteBusyRef = useRef(false);
	const clipboardPendingWriteRef = useRef<PendingRemoteClipboardWrite | null>(null);
	const clipboardCopyReadTimersRef = useRef<number[]>([]);
  ...
  const trackpadCursor = useRef({ x: 0, y: 0, frameWidth: 0, frameHeight: 0, ready: false });
	...
	useEffect(() => () => {
		...
		for (const timer of clipboardCopyReadTimersRef.current) window.clearTimeout(timer);
		clipboardCopyReadTimersRef.current = [];
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (disposed) return;
			...
			if (clipboardSyncEnabledRef.current && clipboardAcknowledgement?.type === "clipboard_read" && clipboardAcknowledgement.id > clipboardLastRemoteAckRef.current) {
				...
				const remoteSequence = Math.max(0, Number(clipboardAcknowledgement.sequence) || 0);
				if (!shouldAcceptRemoteClipboardSequence(clipboardLastRemoteWindowsSequenceRef.current, remoteSequence)) return;
				if (remoteSequence > 0) clipboardLastRemoteWindowsSequenceRef.current = remoteSequence;
				if (clipboardAcknowledgement.error) {
					const pending = clipboardPendingWriteRef.current;
					if (pending && clipboardAcknowledgement.id > pending.gate.afterAckID) {
					...
					clipboardRemoteObservedFingerprintRef.current = textClipboardFingerprint(remoteText);
					const pending = clipboardPendingWriteRef.current;
					if (pending && shouldResolveRemoteClipboardCopy(pending.gate, {
						id: clipboardAcknowledgement.id,
						...
						clipboardLastLocalFingerprintRef.current = textClipboardFingerprint(remoteText);
						return;
					}
					if (pending) return;
			...
			try {
				...
				if (remoteResponse.status === 200) {
					...
					if (sequence > clipboardLastRemoteImageSequenceRef.current && remoteImage.type === "image/png") {
						...
						const hash = await clipboardBlobFingerprint(remoteImage);
	...
	useEffect(() => {
		if (!clipboardSyncEnabled || !navigator.permissions?.query) return;
		let disposed = false;
		let permission: PermissionStatus | null = null;
		const apply = () => {
			if (!disposed && permission?.state === "granted") clipboardBackgroundReadBlockedRef.current = false;
		};
		void navigator.permissions.query({ name: "clipboard-read" as PermissionName }).then((result) => {
			if (disposed) return;
			permission = result;
			apply();
			result.addEventListener("change", apply);
		}).catch(() => {
			// Firefox and Safari intentionally do not expose clipboard permissions.
			// Their trusted paste/copy event paths remain authoritative.
		});
		return () => {
			disposed = true;
	...
	function beginDeferredRemoteClipboardWrite(): boolean {
		...
		const pending: PendingRemoteClipboardWrite = {
			...
			reject: rejectBlob,
			gate: {
				afterAckID: clipboardLastRemoteAckRef.current,
				baselineSequence: clipboardLastRemoteWindowsSequenceRef.current,
				baselineText: clipboardLastRemoteTextRef.current,
	...
	function copyRemoteClipboardFromMac(cut: boolean) {
		...
		clipboardCopyReadTimersRef.current = REMOTE_CLIPBOARD_COPY_READ_DELAYS.map((delay) => window.setTimeout(
	...
	async function writeRemoteClipboardPayload(payload: RemoteClipboardPayload): Promise<void> {
		if (payload.kind === "image") {
			await writeClipboardPNG(payload.image);
			const hash = await clipboardBlobFingerprint(payload.image);
			clipboardLastLocalImageRef.current = hash;
			clipboardLastLocalFingerprintRef.current = imageClipboardFingerprint(hash);
		} else {
			if (!navigator.clipboard?.writeText) throw new Error("Этот браузер не поддерживает запись в системный буфер");
			await navigator.clipboard.writeText(payload.text);
			clipboardLastLocalRef.current = payload.text;
			clipboardLastLocalFingerprintRef.current = textClipboardFingerprint(payload.text);
		}
		clipboardPermissionErrorRef.current = false;
	}

	async function drainRemoteClipboardPayload(): Promise<boolean> {
		if (clipboardRemoteWriteBusyRef.current) return false;
		const pending = clipboardPendingRemoteRef.current;
		if (!pending) return false;
		clipboardRemoteWriteBusyRef.current = true;
		let applied = false;
		try {
			...
			if (newest && !sameRemoteClipboardPayload(newest, pending)) void drainRemoteClipboardPayload();
		}
		return applied;
	}

	function stageRemoteClipboardPayload(payload: RemoteClipboardPayload) {
		clipboardPendingRemoteRef.current = newerRemoteClipboardPayload(clipboardPendingRemoteRef.current, payload);
		setClipboardSyncState("pending");
		void drainRemoteClipboardPayload();
	}

	async function flushPendingRemoteClipboardFromGesture(): Promise<boolean> {
		return drainRemoteClipboardPayload();
	}
	...
	function pasteFromBrowser(event: ReactClipboardEvent<HTMLDivElement>) {
		...
		if (!rawImage && text === "") return;
		event.preventDefault();
		void (async () => pasteLocalClipboardToRemote({
			text,
			image: rawImage ? await normalizeClipboardImage(rawImage) : null,
		}))().catch((reason) => setError(reason instanceof Error ? reason.message : "Не удалось вставить буфер"));
	}
	...
	async function pushLocalClipboardSnapshot(snapshot: LocalClipboardSnapshot) {
		const fingerprint = await localClipboardSnapshotFingerprint(snapshot);
		if (fingerprint === clipboardLastSentToRemoteFingerprintRef.current && fingerprint === clipboardRemoteObservedFingerprintRef.current) return;
		if (snapshot.image) {
			...
			clipboardLastLocalImageRef.current = hash;
			clipboardLastLocalFingerprintRef.current = fingerprint;
			clipboardLastSentToRemoteFingerprintRef.current = fingerprint;
			clipboardPermissionErrorRef.current = false;
			setClipboardSyncState("ready");
			return;
		}
	...
	async function toggleRemoteFullscreen() {
		...
		try {
			...
			// A portrait phone cannot fill itself with a wide Windows monitor without
			// cropping both horizontal edges. Keep the same complete-desktop geometry
			// which was visible immediately before fullscreen; landscape retains the
			// established edge-to-edge presentation.
			setScreenScale(remoteFullscreenScaleMode(compactRemoteClient, remoteViewport.landscape));
			scheduleCamera({ zoom: 1, panX: 0, panY: 0 });
	...
	const workspace = <section ref={workspaceRef} style={remoteViewportStyle} className={`remote-desktop-modal ${embedded ? "remote-desktop-embedded" : ""} ${compactRemoteClient ? "remote-compact-client" : ""} ${controlsCollapsed ? "remote-controls-collapsed" : ""} ${mobileDockHidden ? "remote-mobile-dock-hidden" : ""} ${keyboardOpen ? "remote-keyboard-open" : ""} ${remoteViewport.landscape ? "remote-runtime-landscape" : "remote-runtime-portrait"} ${fullscreenActive ? "remote-is-fullscreen" : ""}`}>
		...
		{filesOpen && <section className="remote-files-overlay" role="dialog" aria-modal="true" aria-label={`Передача файлов · ${device.name}`}>
			<div className="remote-files-dialog">
				<header>
					<div className="remote-files-overlay-title"><span className="remote-files-overlay-icon"><FolderOpen size={20} /></span><span><strong>Передача файлов</strong><small>Моё устройство <span aria-hidden="true">↔</span> {device.name}</small></span></div>
					<div className="remote-files-overlay-actions"><span className="remote-files-overlay-cap"><ShieldCheck size={14} /> Файлы до 50 ГБ</span><button type="button" className="icon-button" onClick={() => setFilesOpen(false)} aria-label="Закрыть файлы"><X size={20} /></button></div>
				</header>
	...
	return <section className="remote-files-card" aria-busy={loading}>
		...
		{!started ? <button className="secondary-button remote-files-open" disabled={!device.online || loading} onClick={() => void openPath("")}><FolderOpen size={17} /> {device.online ? "Открыть проводник" : "Агент не в сети"}</button> : <div className="remote-explorer">
			<section className={`remote-explorer-pane remote-local-pane ${localDragging ? "dragging" : ""}`} onDragEnter={(event) => { event.preventDefault(); setLocalDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setLocalDragging(false); }} onDrop={dropOnLocal}>
				<header><span className="remote-pane-icon"><HardDrive size={17} /></span><span><strong>Моё устройство</strong><small>Файлы для отправки</small></span><button type="button" className="remote-pane-action" disabled={loading} onClick={() => uploadInput.current?.click()}><Plus size={15} /> Добавить</button></header>
				<div className="remote-local-list">
					...
					{localFiles.length === 0 && <button type="button" className="remote-local-empty" disabled={loading} onClick={() => uploadInput.current?.click()}><span className="remote-local-empty-icon"><Upload size={21} /></span><strong>Добавьте файлы</strong><small>Перетащите сюда или выберите на своём устройстве</small><em>Выбрать файлы</em></button>}
				</div>
				<footer><span>{localFiles.length ? `${localFiles.length} выбрано · ${formatBytes(localFiles.reduce((sum, file) => sum + file.size, 0))}` : "Список пока пуст"}</span><button type="button" className="primary-button" disabled={loading || !path || localFiles.length === 0} onClick={() => void uploadFiles(localFiles)}>Отправить на ПК <ChevronRight size={15} /></button></footer>
			</section>
			...
			<section className={`remote-explorer-pane remote-device-pane ${dragging ? "dragging" : ""}`} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDragging(false); }} onDrop={dropOnRemote}>
				...
				<div className="remote-files-toolbar">
					...
					<form className="remote-path" onSubmit={(event) => { event.preventDefault(); void openPath(path); }}><input value={path} onChange={(event) => setPath(event.target.value)} placeholder="C:\\ или /home" aria-label="Путь на удалённом компьютере" /><button className="secondary-button" disabled={loading}>Открыть</button></form>
				</div>
				...
				{!loading && entries.length === 0 && !error && <div className="remote-files-empty"><FolderOpen size={19} /><span><strong>Папка пуста</strong><small>Перетащите сюда файл со своего устройства</small></span></div>}
				<div className="remote-device-drop-hint"><Upload size={15} /> Перетащите сюда — файл попадёт в открытую папку</div>
			</section>
		</div>}
